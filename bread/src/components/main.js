...
//   {
//     name: "바다까까",
//     latlng: "37.5125585,127.1025353",
//   },
//   {
//     name: "동네빵집",
//     latlng: "37.4007821,127.2547115",
//   },
// ];
...
function Main() {
  ...
  useEffect(() => {
    // 데이터 불러온후 카카오 맵에 지도 그릴꺼임
    const container = document.getElementById("map");
    const options = {
      center: new kakao.maps.LatLng(37.5109344, 127.121529),
      level: 10,
    };
    const map = new kakao.maps.Map(container, options);
    // 마커 이미지의 이미지 주소입니다
    var imageSrc = "../public/image.png"; //
    for (var i = 0; i < breadData.length; i++) {
      ...
      console.log(breadData[i].lat, breadData[i].lng);
      var marker = new kakao.maps.Marker({
        map: map, // 마커를 표시할 지도
        position: new kakao.maps.LatLng(breadData[i].lat, breadData[i].lng), // 마커를 표시할 위치
        title: breadData[i].name, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
        image: markerImage, // 마커 이미지
      });
      marker.setMap(map);
      // var iwContent = '<div style="padding:5px;">Hello World!</div>',
      //   iwPosition = new kakao.maps.LatLng(37.4007821, 127.2547115); //인포윈도우 표시 위치입니다
      ...
      let infowindow = new kakao.maps.InfoWindow({
        position: new kakao.maps.LatLng(breadData[i].lat, breadData[i].lng),
        content: `<div style="padding:5px;">${breadData[i].name}</div>`,
      });
    ...
    function BakeryMap() {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch("/bbang.json");
